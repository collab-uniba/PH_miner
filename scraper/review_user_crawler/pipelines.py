        ...
        try:
            ...
            if reviewer is not None:
                try:
                    ...
                    if post:
                        log.msg(
                            'Storing review from %s on product %s into database' % (item['reviewer_name'], post.name))
                        try:
                            review = self.session.query(Review).filter_by(reviewer_id=reviewer.id,
                                                                          post_id=post.id).one()
                            log.msg(
                                "Review from %s on %s already present, updating" % (reviewer.id, post.id))
                            ...
                            log.msg("Adding review from %s on %s" % (reviewer.id, post.id))
                        ...
                        log.msg("No post found in database with url %s, updating" % item['post_url'], _level=log.ERROR)
                except exc.NoResultFound as nrfe:
                    log.msg(str(nrfe), _level=log.ERROR)
        except ProductHuntError as phe:
            log.msg(str(phe), _level=log.ERROR)
...
class ReviewUserCrawlerPipeline(object):
    ...
    def _store_reviewer(self, username):
        log.msg('Storing reviewer %s into database' % username)
        try:
            reviewer = self.ph_client.get_user(username)
            if reviewer is not None:
                try:
                    user = self.session.query(User).filter_by(id=reviewer.id).one()
                    log.msg("Reviewer %s already present, updating" % reviewer.id)
                    ...
                    log.msg("Adding reviewer %s" % reviewer.id)
